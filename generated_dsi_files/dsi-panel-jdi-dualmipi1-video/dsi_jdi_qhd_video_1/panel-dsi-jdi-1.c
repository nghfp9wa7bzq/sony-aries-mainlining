...
{
	return container_of(panel, struct dsi_jdi_1, panel);
}
			       ...
			       struct drm_connector *connector)
{
	...
	mode = drm_mode_duplicate(connector->dev, &dsi_jdi_1_mode);
	if (!mode)
		return -ENOMEM;
...
static int dsi_jdi_1_probe(struct mipi_dsi_device *dsi)
{
	...
	drm_panel_init(&ctx->panel, dev, &dsi_jdi_1_panel_funcs,
