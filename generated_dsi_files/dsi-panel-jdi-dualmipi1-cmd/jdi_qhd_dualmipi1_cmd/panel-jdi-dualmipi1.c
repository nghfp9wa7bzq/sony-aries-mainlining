...
{
	return container_of(panel, struct jdi_dualmipi1, panel);
}
				   ...
				   struct drm_connector *connector)
{
	...
	mode = drm_mode_duplicate(connector->dev, &jdi_dualmipi1_mode);
	if (!mode)
		return -ENOMEM;
...
static int jdi_dualmipi1_probe(struct mipi_dsi_device *dsi)
{
	...
	drm_panel_init(&ctx->panel, dev, &jdi_dualmipi1_panel_funcs,
